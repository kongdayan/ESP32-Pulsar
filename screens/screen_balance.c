 ...
 *
 * 数据由电脑端经 BLE 写入 core/balance_model 的共享存储；本屏每秒刷新一次，
 * 数据缺失或超过 BAL_STALE_MS 未更新时显示 "Waiting for BLE"。
 */
#include "ui.h"
#include "ui_screen.h"
...
static lv_obj_t *row_labels[3] = { NULL, NULL, NULL };
...
static void update_labels(void)
{
    ...
    const bool live = balance_store_get(&data) && data.valid &&
                      !balance_is_stale(&data, now_ms, BAL_STALE_MS);

    if (!live) {
        lv_label_set_text(amount_label, BAL_TEXT_NO_AMOUNT);
        lv_label_set_text(currency_label, BAL_TEXT_WAITING);
...
void screen_balance_init(void)
{
    ...
    currency_label = ui_label_create_aligned(scr, BAL_TEXT_WAITING, role_color(UI_ROLE_TICK),
